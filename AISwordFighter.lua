...
function FindBestAngle(p1, p2, p3)
	...
	v1.FilterDescendantsInstances =
		{ p1, workspace.Filter, p3, table.unpack(p2.Humanoid:GetAccessories()), workspace.Map }
	local v2 = {}
	...
	table.insert(
		...
		{
			workspace:Raycast(
				p1["Right Arm"].Position,
				(p1.PrimaryPart.CFrame * CFrame.Angles(0, -0.017453292519943295, 0)).LookVector.Unit * 10,
				v1
			),
			-1,
		}
	...
	table.insert(
		v2,
		{
			workspace:Raycast(
				p1["Right Arm"].Position,
				(p1.PrimaryPart.CFrame * CFrame.Angles(0, 0.03490658503988659, 0)).LookVector.Unit * 10,
				v1
			),
			2,
		}
...
function GetRetreatDirection(p9, p10, p11)
	...
	local v17 = nil
	for v18 = 1, 10 do
		local v19 = workspace:Raycast(
			p9.PrimaryPart.Position,
			(CFrame.lookAt(p9.PrimaryPart.Position, p9.PrimaryPart.Position + v13) * CFrame.Angles(0, math.rad(v15), 0)).LookVector
				* p11,
			v14
		)
		if v19 then
...
function SideCast(p12)
	...
	local v22 = 0
	for v23 = 1, 10 do
		local v24 = workspace:Raycast(
			p12.PrimaryPart.Position,
			(p12.PrimaryPart.CFrame * CFrame.Angles(0, math.rad(v22), 0)).LookVector * 8,
			v21
		)
		v22 = v22 + 36
		if v24 then
...
function Ai(p1)
	...
	while p1.PrimaryPart and l__Humanoid__7.Health > 0 do
		...
		if v14 and v14.Humanoid.Health > 0 then
			...
			local v21 =
				math.sqrt((l__Position__19.X - l__Position__20.X) ^ 2 + (l__Position__19.Z - l__Position__20.Z) ^ 2)
			...
			if v13 and v17 and v23 and v22 > 6 and v16 then
				...
				p1:SetPrimaryPartCFrame(
					CFrame.lookAt(l__Position__25, v26 - Vector3.new(0, v26.Y - l__Position__25.Y, 0))
						* CFrame.Angles(0, 0.30543261909900765, 0)
				)
			else
				...
				p1:SetPrimaryPartCFrame(
					CFrame.lookAt(
						l__PrimaryPart__8.Position,
						v15.Position - Vector3.new(0, v15.Position.Y - l__PrimaryPart__8.Position.Y, 0)
					) * CFrame.Angles(0, 0.30543261909900765, 0)
				)
				local v27 = 0
				local v28 = v27
				for v29 = 1, 10 do
					...
					if v28 == v27 then
						...
						p1:SetPrimaryPartCFrame(
							CFrame.lookAt(
								l__PrimaryPart__8.Position,
								v15.Position - Vector3.new(0, v15.Position.Y - l__PrimaryPart__8.Position.Y, 0)
							) * CFrame.Angles(0, math.rad(17.5 + v30), 0)
						)
						break
					end
					v28 = v27
					p1:SetPrimaryPartCFrame(
						CFrame.lookAt(
							l__PrimaryPart__8.Position,
							v15.Position - Vector3.new(0, v15.Position.Y - l__PrimaryPart__8.Position.Y, 0)
						) * CFrame.Angles(0, math.rad(17.5 + v27), 0)
					)
				end
			end
			local v31 = ((p1.PrimaryPart.Position - v14.PrimaryPart.Position).Unit - v14.Humanoid.MoveDirection).Magnitude
				< 1
			if v13 then
				local v32 = 8
			else
