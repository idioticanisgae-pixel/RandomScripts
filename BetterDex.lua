...
local set_ro = setreadonly or (make_writeable and function(t, v) 
     if v 
        then 
         make_readonly(t) 
    else 
...
context:Register("UPVALUE_EDITOR", {Name = "Upvalue Editor [RE]", IconMap = Explorer.MiscIcons, Icon = "ExploreData", OnClick = function()
    ...
    if not getgcf or not getupvalues or not setupvalue then
        if getgenv().DoNotif then getgenv().DoNotif("⚠ Upvalue editor requires getgc/getupvalues/setupvalue", 3) end
        return
    end
    local ok, gc = pcall(getgcf)
    if not ok or not gc then
        if getgenv().DoNotif then getgenv().DoNotif("⚠ getgc() failed", 2) end
        return
    end
    local closures = {}
    for _, fn in pairs(gc) do
        if type(fn) == "function" then
            ...
            if ok2 and fenv and (rawget(fenv, "script") == scr or (type(fenv) == "table" and fenv.script == scr)) then
                table.insert(closures, fn)
            end
        end
    end
    if #closures == 0 then
        if getgenv().DoNotif then getgenv().DoNotif("⚠ No closures found for: " .. scr.Name, 2) end
        return
    end
    local entries = {}
    local seen = {}
        ...
        if ok3 and ups then
            for i, val in ipairs(ups) do
                ...
                if not seen[key] then
                    ...
                    table.insert(entries, {fn = fn, idx = i, name = name, val = val, vtype = type(val)})
                end
            end
        end
    end
    if #entries == 0 then
        if getgenv().DoNotif then getgenv().DoNotif("⚠ No upvalues found on " .. #closures .. " closure(s)", 2) end
        return
    end
    ...
    local function setStatus(msg, isErr)
        statusBar.Text = "  " .. msg
        statusBar.TextColor3 = isErr and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(100, 255, 140)
    end
    local function safeDisplayVal(v)
        local t = type(v)
        if t == "string"  then return '"' .. v:sub(1, 50):gsub('"', '\\"'):gsub("\n","\\n") .. (v:len()>50 and '...' or '') .. '"' end
        if t == "number"  then return tostring(v) end
        if t == "boolean" then return tostring(v) end
        if t == "table"   then return "[table #" .. tostring(#v) .. "]" end
        if t == "function" then return "[function]" end
        if t == "userdata" then local ok, s = pcall(tostring, v); return ok and s or "[userdata]" end
        return "[" .. t .. "]"
    end
    for i, entry in ipairs(entries) do
        ...
        row.BackgroundColor3 = i % 2 == 0 and Color3.fromRGB(26,26,34) or Color3.fromRGB(30,30,40)
        ...
        if entry.vtype == "number" or entry.vtype == "string" or entry.vtype == "boolean" then
            ...
            setBtn.MouseButton1Click:Connect(function()
                ...
                if entry.vtype == "number" then
                    newVal = tonumber(raw)
                    if newVal == nil then setStatus("⚠ Invalid number: " .. raw, true) return end
                elseif entry.vtype == "boolean" then
                    if raw == "true" then newVal = true
                    elseif raw == "false" then newVal = false
                    else setStatus("⚠ Must be true or false", true) return end
                else
                    newVal = raw
                end
                ...
                if ok then
                    ...
                    task.delay(1.2, function() if setBtn and setBtn.Parent then setBtn.BackgroundColor3 = Color3.fromRGB(60, 120, 240) end end)
                else
    ...
    if getgenv().DoNotif then getgenv().DoNotif("Upvalue Editor: " .. #entries .. " upvalues on " .. scr.Name, 3) end
end})
context:Register("TABLE_INSPECTOR", {Name = "Table Edit [RE]", IconMap = Explorer.MiscIcons, Icon = "ExploreData", OnClick = function()
	local node = selection.List[1]
	if not node then return end
	local obj = node.Obj
	local getgcf   = env.getgc or getgc or get_gc_objects
	...
	if pcall(function() return obj:IsA("LuaSourceContainer") end) and obj:IsA("LuaSourceContainer") then
		if getgcf then
			local ok, gc = pcall(getgcf)
			if ok and gc then
				for _, fn in pairs(gc) do
					if type(fn) == "function" then
						local ok2, fenv = pcall(getfenv_, fn)
						if ok2 and fenv and (rawget(fenv, "script") == obj or (type(fenv) == "table" and fenv.script == obj)) then
							rootTable = fenv
		...
		if not rootTable then
			if pcall(function() return obj:IsA("ModuleScript") end) and obj:IsA("ModuleScript") then
		...
		if not rootTable then
			if getgenv().DoNotif then getgenv().DoNotif("⚠ No table found for: " .. obj.Name, 2) end
			return
		end
	else
	...
	local function _unlock(t) pcall(function()
		if setreadonly then setreadonly(t, false) elseif make_writeable then make_writeable(t) end
	end) end
	local function _lock(t)   pcall(function()
		if setreadonly then setreadonly(t, true) end
	end) end

	local function applyPatch(tbl, key, val, isFunc)
		if type(tbl) ~= "table" then return false end
		if not activePatches[tbl] then activePatches[tbl] = {} end
		activePatches[tbl][key] = {Value = val, Locked = true, IsFunction = isFunc}
		pcall(function()
			_unlock(tbl)
			if isFunc then
				if val == "TRUE"      then rawset(tbl, key, function() return true  end)
				elseif val == "FALSE" then rawset(tbl, key, function() return false end)
		...
		if mt then pcall(function()
			_unlock(mt)
			local oldIdx = rawget(mt, "__index")
			rawset(mt, "__index", function(t, k)
				if k == key then return val
				elseif type(oldIdx) == "function" then return oldIdx(t, k)
				elseif type(oldIdx) == "table"    then return oldIdx[k] end
			end)
			_lock(mt)
		end) end
		return true
	...
	local function hookValue(tbl, key, value)
		...
		if valueHooks[hk] then valueHooks[hk].enabled = true; return end
		valueHooks[hk] = {table=tbl, key=key, value=value, enabled=true}
		pcall(function() _unlock(tbl); rawset(tbl, key, value); _lock(tbl) end)
		local mt = _getRawMt(tbl)
		if mt then pcall(function()
			_unlock(mt)
			local origNI = rawget(mt, "__newindex")
			rawset(mt, "__newindex", function(t, k, v)
				if k == key then rawset(tbl, key, value)
				elseif type(origNI) == "function" then origNI(t, k, v)
		...
		hookedConns[hk] = game:GetService("RunService").Heartbeat:Connect(function()
			if valueHooks[hk] and valueHooks[hk].enabled then
				pcall(function() if rawget(tbl, key) ~= value then rawset(tbl, key, value) end end)
			end
		end)
	end
	...
	local function unhookValue(tbl, key)
		local hk = tostring(tbl) .. ":" .. tostring(key)
		valueHooks[hk] = nil
		if hookedConns[hk] then pcall(function() hookedConns[hk]:Disconnect() end); hookedConns[hk] = nil end
	end
	...
	local function getUpvalues(func, depth, maxDepth)
		...
		if ok and result then
			for i, uv in ipairs(result) do
				local t = type(uv)
				uvs[i] = {
					Index=i, Value=uv, Type=t,
					IsFunction = t=="function", IsTable = t=="table",
					ChildUpvalues = (t=="function") and getUpvalues(uv, depth+1, maxDepth) or {}
				}
			end
		end
		return uvs
	...
	local function getPatchCount()
		local n = 0; for _ in pairs(activePatches) do n = n + 1 end; return n
	end
	...
	closeBtn.MouseButton1Click:Connect(function()
		for _, conn in pairs(hookedConns) do pcall(function() conn:Disconnect() end) end
		screenGui:Destroy()
	end)
	closeBtn.MouseEnter:Connect(function() closeBtn.BackgroundTransparency = 0; closeBtn.BackgroundColor3 = Color3.fromRGB(180,40,40) end)
	...
	backBtn.MouseEnter:Connect(function() backBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 60) end)
	backBtn.MouseLeave:Connect(function() backBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 35) end)
	...
	searchBox.Focused:Connect(function() searchStroke.Color = Color3.fromRGB(0, 200, 150) end)
	searchBox.FocusLost:Connect(function() searchStroke.Color = Color3.fromRGB(40, 40, 50) end)
	...
	local function updateTitle()
		titleLabel.Text = "TABLE INSPECTOR — " .. rootLabel
			.. "  [" .. getPatchCount() .. " patches]"
	end

	local function updateBreadcrumb()
		local parts = {}
		for _, s in ipairs(navStack) do table.insert(parts, s.label) end
	...
	local function safeVal(v)
		...
		if t == "number"  then return v==math.floor(v) and tostring(math.floor(v)) or string.format("%.6g",v) end
		if t == "boolean" then return tostring(v) end
		if t == "function" then
			local ok3, info = pcall(function() return debug and debug.getinfo and debug.getinfo(v,"S") end)
			if ok3 and info and info.short_src then
				return "[fn:"..info.short_src..":"..(info.linedefined or "?").."]"
			end
			return "[function]"
		end
		if t == "thread" then return "[thread:"..(tostring(v):match("0x%x+") or "?").."]" end
		if t == "table" then
			local arr = #v; local total = 0; for _ in pairs(v) do total=total+1 end
			local ok4,mt = pcall(getmetatable, v)
			return "{arr="..arr.." total="..total..(ok4 and type(mt)=="table" and " +mt" or "").."}"
		end
		local ok2,s = pcall(tostring,v); return ok2 and s or ("["..t.."]")
	end
	...
	local function showCodePanel(title, func, editable)
		...
		overlay.BackgroundTransparency = 0.5; overlay.BorderSizePixel = 0; overlay.ZIndex = 60

		local panel = Instance.new("Frame", screenGui)
		panel.Size = UDim2.new(0,560,0,380); panel.Position = UDim2.new(0.5,-280,0.5,-190)
		panel.BackgroundColor3 = Color3.fromRGB(10,10,12); panel.BorderSizePixel = 0; panel.ZIndex = 61
		local pStroke = Instance.new("UIStroke", panel)
		pStroke.Color = Color3.fromRGB(0,200,150); pStroke.Thickness = 1

		local pTitle = Instance.new("TextLabel", panel)
		pTitle.Size = UDim2.new(1,-36,0,22); pTitle.BackgroundTransparency = 1
		pTitle.Font = Enum.Font.GothamBold; pTitle.TextSize = 11
		pTitle.TextColor3 = Color3.fromRGB(0,255,170); pTitle.TextXAlignment = Enum.TextXAlignment.Left
		pTitle.Position = UDim2.new(0,8,0,0); pTitle.Text = title; pTitle.ZIndex = 62

		local pClose = Instance.new("TextButton", panel)
		pClose.Size = UDim2.new(0,20,0,20); pClose.Position = UDim2.new(1,-24,0,1)
		pClose.BackgroundTransparency = 1; pClose.Font = Enum.Font.GothamBold
		pClose.Text = "X"; pClose.TextColor3 = Color3.new(1,1,1); pClose.TextSize = 12; pClose.ZIndex = 62
		pClose.MouseButton1Click:Connect(function() overlay:Destroy(); panel:Destroy() end)

		local codeBox = Instance.new("TextBox", panel)
		codeBox.Size = UDim2.new(1,-8,1,-30); codeBox.Position = UDim2.new(0,4,0,24)
		codeBox.BackgroundColor3 = Color3.fromRGB(15,15,18)
		codeBox.Font = Enum.Font.Code; codeBox.TextSize = 10
		codeBox.TextColor3 = Color3.fromRGB(0,220,160); codeBox.TextXAlignment = Enum.TextXAlignment.Left
		codeBox.TextYAlignment = Enum.TextYAlignment.Top
		codeBox.MultiLine = true; codeBox.TextWrapped = false
		codeBox.ClearTextOnFocus = false; codeBox.TextEditable = editable or false
		codeBox.Text = "-- decompiling..."; codeBox.ZIndex = 62
	...
	local function showUpvaluesPanel(func, funcName)
		...
		local panel = Instance.new("Frame", screenGui)
		panel.Size = UDim2.new(0,440,0,360); panel.Position = UDim2.new(0.5,-220,0.5,-180)
		panel.BackgroundColor3 = Color3.fromRGB(10,10,12); panel.BorderSizePixel = 0; panel.ZIndex = 61
		local pStroke = Instance.new("UIStroke", panel)
		pStroke.Color = Color3.fromRGB(150,100,255); pStroke.Thickness = 1

		local pTitle = Instance.new("TextLabel", panel)
		pTitle.Size = UDim2.new(1,-36,0,22); pTitle.BackgroundTransparency = 1
		pTitle.Font = Enum.Font.GothamBold; pTitle.TextSize = 11
		pTitle.TextColor3 = Color3.fromRGB(150,100,255); pTitle.TextXAlignment = Enum.TextXAlignment.Left
		pTitle.Position = UDim2.new(0,8,0,0)
		pTitle.Text = "UPVALUES: " .. funcName; pTitle.ZIndex = 62
		...
		uvScroll.Size = UDim2.new(1,-8,1,-28); uvScroll.Position = UDim2.new(0,4,0,26)
		uvScroll.BackgroundColor3 = Color3.fromRGB(15,15,18); uvScroll.BorderSizePixel = 0
		uvScroll.ScrollBarThickness = 3; uvScroll.ScrollBarImageColor3 = Color3.fromRGB(150,100,255)
		uvScroll.CanvasSize = UDim2.new(0,0,0,0); uvScroll.ZIndex = 62
		...
		if #uvs == 0 then
			...
			lbl.Text = "  -- no upvalues found --"; lbl.ZIndex = 63
		else
			for _, uvData in ipairs(uvs) do
				local uvRow = Instance.new("Frame", uvScroll)
				uvRow.Size = UDim2.new(1,-4,0,30); uvRow.BackgroundTransparency = 1; uvRow.ZIndex = 62

				local uvLbl = Instance.new("TextLabel", uvRow)
				uvLbl.Size = UDim2.new(0,160,1,0); uvLbl.BackgroundTransparency = 1
				uvLbl.Font = Enum.Font.Code; uvLbl.TextSize = 9
				uvLbl.TextColor3 = Color3.fromRGB(100,200,255)
				uvLbl.TextXAlignment = Enum.TextXAlignment.Left; uvLbl.ClipsDescendants = true
				uvLbl.Text = "  [" .. uvData.Index .. "] " .. uvData.Type; uvLbl.ZIndex = 63

				if uvData.IsTable then
					local diveBtn = Instance.new("TextButton", uvRow)
					diveBtn.Size = UDim2.new(0,90,0,22); diveBtn.Position = UDim2.new(0,162,0,4)
					diveBtn.BackgroundColor3 = Color3.fromRGB(30,60,80)
					diveBtn.Text = "DIVE UV >"; diveBtn.TextColor3 = Color3.fromRGB(0,200,255)
					diveBtn.Font = Enum.Font.Code; diveBtn.TextSize = 8
					...
					diveBtn.MouseButton1Click:Connect(function()
						...
						table.insert(navStack, {tbl = uvData.Value, label = "[UV:"..uvData.Index.."]"})
						currentPage = 1; updateBreadcrumb()
						searchBox.Text = ""
				...
				elseif uvData.IsFunction then
					...
					uvBtn.Size = UDim2.new(0,60,0,22); uvBtn.Position = UDim2.new(0,162,0,4)
					uvBtn.BackgroundColor3 = Color3.fromRGB(60,40,100)
					uvBtn.Text = "UVALS"; uvBtn.TextColor3 = Color3.fromRGB(150,100,255)
					uvBtn.Font = Enum.Font.Code; uvBtn.TextSize = 8
					...
					viewBtn.Size = UDim2.new(0,50,0,22); viewBtn.Position = UDim2.new(0,228,0,4)
					viewBtn.BackgroundColor3 = Color3.fromRGB(60,40,60)
					viewBtn.Text = "VIEW"; viewBtn.TextColor3 = Color3.fromRGB(255,100,255)
					viewBtn.Font = Enum.Font.Code; viewBtn.TextSize = 8
					...
					editBox.Size = UDim2.new(0,120,0,22); editBox.Position = UDim2.new(0,162,0,4)
					...
					editBox.Font = Enum.Font.Code; editBox.TextSize = 9
	...
	local function createTableRow(tbl, k, v)
		...
		-- hover tint
		row.MouseEnter:Connect(function() row.BackgroundColor3 = Color3.fromRGB(18,22,28); row.BackgroundTransparency = 0 end)
		...
		keyLbl.Font = Enum.Font.Code; keyLbl.TextSize = 9
		...
		if vt == "table" then
			...
			diveBtn.Size = UDim2.new(0,80,0,24); diveBtn.Position = UDim2.fromScale(0.34, 0.14)
			diveBtn.BackgroundColor3 = Color3.fromRGB(20,40,60)
			diveBtn.Text = "DIVE >"; diveBtn.TextColor3 = Color3.fromRGB(0,255,170)
			diveBtn.Font = Enum.Font.Code; diveBtn.TextSize = 8
			Instance.new("UICorner", diveBtn).CornerRadius = UDim.new(0,2)
			diveBtn.MouseButton1Click:Connect(function()
				table.insert(navStack, {tbl = v, label = ks})
				currentPage = 1; updateBreadcrumb(); searchBox.Text = ""
				renderTable(v, "")
			...
			if okMt and type(mt) == "table" then
				...
				mtBtn.Size = UDim2.new(0,26,0,24); mtBtn.Position = UDim2.fromScale(0.48, 0.14)
				mtBtn.BackgroundColor3 = Color3.fromRGB(60,20,80)
				mtBtn.Text = "MT"; mtBtn.TextColor3 = Color3.fromRGB(200,100,255)
				mtBtn.Font = Enum.Font.Code; mtBtn.TextSize = 7
			...
			prevLbl.Size = UDim2.new(0,120,1,0); prevLbl.Position = UDim2.fromScale(0.55,0)
			prevLbl.BackgroundTransparency = 1; prevLbl.Font = Enum.Font.Code; prevLbl.TextSize = 8
			prevLbl.TextColor3 = TYPE_COLORS["table"]; prevLbl.TextXAlignment = Enum.TextXAlignment.Left
			prevLbl.ClipsDescendants = true; prevLbl.Text = safeVal(v)

		elseif vt == "function" then
			-- SPOOF button (toggle NORMAL/TRUE/FALSE)
			local spoofBtn = Instance.new("TextButton", row)
			spoofBtn.Size = UDim2.new(0,40,0,24); spoofBtn.Position = UDim2.fromScale(0.34, 0.14)
			spoofBtn.BackgroundColor3 = Color3.fromRGB(50,50,70)
			spoofBtn.Text = "SPOOF"; spoofBtn.TextColor3 = Color3.new(1,1,1)
			spoofBtn.Font = Enum.Font.Code; spoofBtn.TextSize = 7
			...
			spoofBtn.MouseButton1Click:Connect(function()
				...
				spoofBtn.BackgroundColor3 = mode=="TRUE" and Color3.fromRGB(0,160,80)
					or mode=="FALSE" and Color3.fromRGB(180,40,40) or Color3.fromRGB(50,50,70)
				if mode == "NORMAL" then
					if activePatches[tbl] then activePatches[tbl][k] = nil end
				else
			...
			uvBtn.Size = UDim2.new(0,38,0,24); uvBtn.Position = UDim2.fromScale(0.42, 0.14)
			uvBtn.BackgroundColor3 = Color3.fromRGB(60,40,100)
			uvBtn.Text = "UVALS"; uvBtn.TextColor3 = Color3.fromRGB(150,100,255)
			uvBtn.Font = Enum.Font.Code; uvBtn.TextSize = 7
			Instance.new("UICorner", uvBtn).CornerRadius = UDim.new(0,2)
			uvBtn.MouseButton1Click:Connect(function() showUpvaluesPanel(v, ks) end)
			-- VIEW button
			local viewBtn = Instance.new("TextButton", row)
			viewBtn.Size = UDim2.new(0,28,0,24); viewBtn.Position = UDim2.fromScale(0.50, 0.14)
			viewBtn.BackgroundColor3 = Color3.fromRGB(60,40,60)
			viewBtn.Text = "VIEW"; viewBtn.TextColor3 = Color3.fromRGB(255,100,255)
			viewBtn.Font = Enum.Font.Code; viewBtn.TextSize = 7
			Instance.new("UICorner", viewBtn).CornerRadius = UDim.new(0,2)
			viewBtn.MouseButton1Click:Connect(function() showCodePanel("VIEW: "..ks, v, false) end)
			-- EDIT button
			local editBtn = Instance.new("TextButton", row)
			editBtn.Size = UDim2.new(0,28,0,24); editBtn.Position = UDim2.fromScale(0.57, 0.14)
			editBtn.BackgroundColor3 = Color3.fromRGB(100,70,50)
			editBtn.Text = "EDIT"; editBtn.TextColor3 = Color3.fromRGB(255,180,100)
			editBtn.Font = Enum.Font.Code; editBtn.TextSize = 7
			Instance.new("UICorner", editBtn).CornerRadius = UDim.new(0,2)
			editBtn.MouseButton1Click:Connect(function() showCodePanel("EDIT: "..ks, v, true) end)
			-- fn preview
			local fnLbl = Instance.new("TextLabel", row)
			fnLbl.Size = UDim2.new(0,80,1,0); fnLbl.Position = UDim2.fromScale(0.66,0)
			fnLbl.BackgroundTransparency = 1; fnLbl.Font = Enum.Font.Code; fnLbl.TextSize = 8
			fnLbl.TextColor3 = TYPE_COLORS["function"]; fnLbl.TextXAlignment = Enum.TextXAlignment.Left
			fnLbl.ClipsDescendants = true; fnLbl.Text = safeVal(v)

		else
			-- Primitive: inline TextBox edit
			local box = Instance.new("TextBox", row)
			box.Size = UDim2.new(0,100,0,24); box.Position = UDim2.fromScale(0.34, 0.14)
			...
			if vt == "number" then
				...
				bStroke.Thickness = 1; bStroke.Color = Color3.fromRGB(40,40,50)
				box:GetPropertyChangedSignal("Text"):Connect(function()
					bStroke.Color = tonumber(box.Text) and Color3.fromRGB(40,160,80) or Color3.fromRGB(200,60,60)
				end)
			end
			...
			if vt == "number" then
				...
				hookBtn.Size = UDim2.new(0,40,0,24); hookBtn.Position = UDim2.fromScale(0.53, 0.14)
				hookBtn.BackgroundColor3 = Color3.fromRGB(20,50,30)
				hookBtn.Text = "HOOK"; hookBtn.TextColor3 = Color3.fromRGB(0,200,150)
				hookBtn.Font = Enum.Font.Code; hookBtn.TextSize = 7
				...
				unhookBtn.Size = UDim2.new(0,40,0,24); unhookBtn.Position = UDim2.fromScale(0.62, 0.14)
				unhookBtn.BackgroundColor3 = Color3.fromRGB(50,20,20)
				unhookBtn.Text = "UNHK"; unhookBtn.TextColor3 = Color3.fromRGB(255,100,100)
				unhookBtn.Font = Enum.Font.Code; unhookBtn.TextSize = 7
			...
			-- COPY button
			local setclip = (env and env.setclipboard) or (pcall(function() return setclipboard end) and setclipboard) or nil
			if setclip then
				local copyBtn = Instance.new("TextButton", row)
				copyBtn.Size = UDim2.new(0,26,0,24); copyBtn.Position = UDim2.fromScale(0.72, 0.14)
				copyBtn.BackgroundColor3 = Color3.fromRGB(30,30,50)
				copyBtn.Text = "CP"; copyBtn.TextColor3 = Color3.fromRGB(150,180,255)
				copyBtn.Font = Enum.Font.Code; copyBtn.TextSize = 7
			...
			delBtn.Size = UDim2.new(0,22,0,24); delBtn.Position = UDim2.fromScale(0.80, 0.14)
			...
			delBtn.MouseButton1Click:Connect(function()
				if not deleteArmed then
					deleteArmed = true; delBtn.Text = "?!"; delBtn.BackgroundColor3 = Color3.fromRGB(180,100,0)
					task.delay(1.5, function()
						if deleteArmed then deleteArmed=false; delBtn.Text="X"; delBtn.BackgroundColor3=Color3.fromRGB(100,25,25) end
					end)
				else
					rawset(tbl, k, nil)
	...
	renderTable = function(tbl, filter)
		...
		if sortMode == "key_asc" or sortMode == "key_desc" then
			...
			table.sort(keys, function(a, b)
				...
				if ta == tb2 then
					if ta == "number" then return asc and a<b or a>b end
		...
		elseif sortMode == "type" then
			table.sort(keys, function(a, b)
				local ok1, va = pcall(rawget, tbl, a); if not ok1 or va==nil then va = tbl[a] end
				local ok2, vb = pcall(rawget, tbl, b); if not ok2 or vb==nil then vb = tbl[b] end
				local ta2, tb3 = type(va), type(vb)
				if ta2==tb3 then return tostring(a)<tostring(b) end
				return ta2<tb3
			end)
		...
		for _, k in ipairs(keys) do
			...
			local vt = type(v)
			if currentTypeFilter and vt ~= currentTypeFilter then continue end
			if filter and filter ~= "" then
				local fLow = filter:lower()
				if not tostring(k):lower():find(fLow, 1, true) and not safeVal(v):lower():find(fLow, 1, true) then
					continue
				end
			end
		...
		if mt then
			if type(mt.__index) == "table" then
				...
				sep.Font = Enum.Font.Code; sep.TextSize = 9
				sep.TextColor3 = Color3.fromRGB(255,0,255)
				sep.Text = "  -- GHOST INDEX (__index) --"; sep.LayoutOrder = 900000
			...
			if #mms > 0 then
				...
				mmSep.Size = UDim2.new(1,0,0,18); mmSep.BackgroundTransparency = 1
				mmSep.Font = Enum.Font.Code; mmSep.TextSize = 9
				mmSep.TextColor3 = Color3.fromRGB(255,200,100)
				mmSep.Text = "  -- METAMETHODS --"; mmSep.LayoutOrder = 900001
				for _, mmData in ipairs(mms) do
					local mmRow = Instance.new("Frame", scroll)
					mmRow.Size = UDim2.new(1,-4,0,35); mmRow.BackgroundTransparency = 1; mmRow.LayoutOrder = 900002
					local mmLbl = Instance.new("TextLabel", mmRow)
					mmLbl.Size = UDim2.new(0.32,0,1,0); mmLbl.BackgroundTransparency = 1
					mmLbl.Font = Enum.Font.Code; mmLbl.TextSize = 9
					mmLbl.TextColor3 = Color3.fromRGB(255,200,100)
					mmLbl.TextXAlignment = Enum.TextXAlignment.Left; mmLbl.ClipsDescendants = true
					mmLbl.Text = "  " .. mmData.Key .. "()"
					local mmUv = Instance.new("TextButton", mmRow)
					mmUv.Size = UDim2.new(0,50,0,24); mmUv.Position = UDim2.fromScale(0.34,0.14)
					mmUv.BackgroundColor3 = Color3.fromRGB(60,40,100)
					mmUv.Text = "UVALS"; mmUv.TextColor3 = Color3.fromRGB(150,100,255)
					mmUv.Font = Enum.Font.Code; mmUv.TextSize = 7
					Instance.new("UICorner", mmUv).CornerRadius = UDim.new(0,2)
					mmUv.MouseButton1Click:Connect(function() showUpvaluesPanel(mmData.Value, mmData.Key.."()") end)
					local mmView = Instance.new("TextButton", mmRow)
					mmView.Size = UDim2.new(0,45,0,24); mmView.Position = UDim2.fromScale(0.44,0.14)
					mmView.BackgroundColor3 = Color3.fromRGB(60,40,60)
					mmView.Text = "VIEW"; mmView.TextColor3 = Color3.fromRGB(255,100,255)
					mmView.Font = Enum.Font.Code; mmView.TextSize = 7
					Instance.new("UICorner", mmView).CornerRadius = UDim.new(0,2)
					mmView.MouseButton1Click:Connect(function() showCodePanel(mmData.Key.."()", mmData.Value, false) end)
				end
			end
		end
		...
		if totalPages > 1 then
			...
			pageRow.Size = UDim2.new(1,0,0,26); pageRow.BackgroundColor3 = Color3.fromRGB(15,15,18)
			pageRow.BorderSizePixel = 0; pageRow.LayoutOrder = 999998
			local prevBtn = Instance.new("TextButton", pageRow)
			prevBtn.Size = UDim2.new(0,55,0,20); prevBtn.Position = UDim2.new(0.5,-80,0.5,-10)
			prevBtn.BackgroundColor3 = currentPage>1 and Color3.fromRGB(30,30,38) or Color3.fromRGB(20,20,24)
			prevBtn.Text = "< PREV"; prevBtn.Font = Enum.Font.Code; prevBtn.TextSize = 9
			prevBtn.TextColor3 = currentPage>1 and Color3.fromRGB(0,200,150) or Color3.fromRGB(50,50,50)
			prevBtn.BorderSizePixel = 0
			prevBtn.MouseButton1Click:Connect(function()
				if currentPage>1 then currentPage=currentPage-1; renderTable(navStack[#navStack].tbl, searchBox.Text) end
			end)
			local pageInfo = Instance.new("TextLabel", pageRow)
			pageInfo.Size = UDim2.new(0,70,1,0); pageInfo.Position = UDim2.new(0.5,-35,0,0)
			pageInfo.BackgroundTransparency = 1; pageInfo.Font = Enum.Font.Code; pageInfo.TextSize = 9
			pageInfo.TextColor3 = Color3.fromRGB(100,100,100)
			pageInfo.Text = currentPage .. " / " .. totalPages
			local nextBtn = Instance.new("TextButton", pageRow)
			nextBtn.Size = UDim2.new(0,55,0,20); nextBtn.Position = UDim2.new(0.5,25,0.5,-10)
			nextBtn.BackgroundColor3 = currentPage<totalPages and Color3.fromRGB(30,30,38) or Color3.fromRGB(20,20,24)
			nextBtn.Text = "NEXT >"; nextBtn.Font = Enum.Font.Code; nextBtn.TextSize = 9
			nextBtn.TextColor3 = currentPage<totalPages and Color3.fromRGB(0,200,150) or Color3.fromRGB(50,50,50)
			nextBtn.BorderSizePixel = 0
			nextBtn.MouseButton1Click:Connect(function()
				if currentPage<totalPages then currentPage=currentPage+1; renderTable(navStack[#navStack].tbl, searchBox.Text) end
			end)
		end

		scroll.CanvasSize = UDim2.new(0,0,0, (rowCount + 4) * 35 + 60)
		local filterSuffix = currentTypeFilter and ("  [type:"..currentTypeFilter.."]") or ""
		local pageSuffix = totalPages>1 and ("  [pg "..currentPage.."/"..totalPages.."]") or ""
		statusLabel.Text = totalFiltered .. " / " .. totalKeys .. " keys" .. filterSuffix .. pageSuffix
	end

	-- wire up UV dive deferred call
	local uvDiveConn; uvDiveConn = game:GetService("RunService").Heartbeat:Connect(function()
	...
	-- ── type filter buttons ──────────────────────────────────────────────
	do
		local btnW = 62; local xOff = 4
		for i, tname in ipairs(TYPE_NAMES) do
			local tfBtn = Instance.new("TextButton", filterBar)
			tfBtn.Size = UDim2.new(0,btnW,0,18); tfBtn.Position = UDim2.new(0, xOff + (i-1)*(btnW+3), 0.5,-9)
			tfBtn.BackgroundColor3 = Color3.fromRGB(30,30,38)
			tfBtn.Text = tname:upper(); tfBtn.Font = Enum.Font.Code; tfBtn.TextSize = 8
			...
			tfBtn.MouseButton1Click:Connect(function()
				if currentTypeFilter == tname then
					...
					tfBtn.BackgroundColor3 = TYPE_COLORS[tname] and Color3.fromRGB(30,50,40) or Color3.fromRGB(40,40,60)
		...
		allBtn.Size = UDim2.new(0,38,0,18); allBtn.Position = UDim2.new(0, xOff + #TYPE_NAMES*(btnW+3), 0.5,-9)
		allBtn.BackgroundColor3 = Color3.fromRGB(60,50,20)
		allBtn.Text = "ALL"; allBtn.Font = Enum.Font.Code; allBtn.TextSize = 8
		allBtn.TextColor3 = Color3.fromRGB(255,200,100); allBtn.BorderSizePixel = 0
	...
	colHeader.Size = UDim2.new(1,0,0,16); colHeader.Position = UDim2.new(0,0,0,74)
	colHeader.BackgroundColor3 = Color3.fromRGB(15,15,18); colHeader.BorderSizePixel = 0
	local function makeSortHdr(label, modeA, modeB, x, w)
		local btn = Instance.new("TextButton", colHeader)
		btn.Size = UDim2.new(0,w,1,0); btn.Position = UDim2.new(0,x,0,0)
		btn.BackgroundTransparency = 1; btn.AutoButtonColor = false
		btn.Font = Enum.Font.GothamBold; btn.TextSize = 9
		btn.TextXAlignment = Enum.TextXAlignment.Left
		btn.Text = "  " .. label
		btn.TextColor3 = (sortMode==modeA or sortMode==modeB) and Color3.fromRGB(0,255,170) or Color3.fromRGB(80,80,80)
	...
	if getgenv().DoNotif then getgenv().DoNotif("Table Inspector: " .. rootLabel, 3) end

	-- cleanup UV dive conn when gui destroyed
	screenGui.AncestryChanged:Connect(function()
		if not screenGui.Parent then
			pcall(function() uvDiveConn:Disconnect() end)
			for _, conn in pairs(hookedConns) do pcall(function() conn:Disconnect() end) end
		end
	end)
end})
...
local function requestNeuralLink(prompt: string, maxTokens: number): string?
	...
	return nil
				...
				if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
					...
					if okBC and bytecode and bytecode ~= "" then
						local opts = {
							...
							ShowTrivialOperations=false, ShowInstructionLines=false,
							ShowOperationIndex=false, ShowOperationNames=false,
							ListUsedGlobals=true, UseTypeInfo=false,
		...
		ScriptViewer.ViewRaw(output)
		if getgenv().DoNotif then
		getgenv().DoNotif("✓ Universal poison opened in notepad! (" .. patchedCount .. " patches)", 3)
		end
		end
		})
    ...
    elseif n == "BulletLifetime" then return 10
		...
		-- Open output in ScriptViewer notepad
		ScriptViewer.ViewRaw(output)
		if getgenv().DoNotif then getgenv().DoNotif("✓ Poison Patch opened in notepad!", 3) end
		end})
		...
		OnClick = function()
			...
			local function serialize(v)
				...
				elseif t == "CFrame" then
					...
					if v == CFrame.identity then return "CFrame.identity" end
					local c = {v:GetComponents()}
					local strs = {}
					for _, n in ipairs(c) do strs[#strs+1] = fmtNum(n) end
				...
				elseif t == "Rect" then
					return ("Rect.new(%s, %s, %s, %s)"):format(
						fmtNum(v.Min.X), fmtNum(v.Min.Y), fmtNum(v.Max.X), fmtNum(v.Max.Y))
			...
			local IMAGE_COMMON = {
				...
				"ImageRectSize","ResampleMode","ScaleType","SliceCenter","SliceScale","TileSize",
			}
			...
			local propertyMap = {
				ScreenGui = {
					...
					"ZIndexBehavior","ScreenInsets","SafeAreaCompatibility",
				},
				TextLabel = dedup(merge(COMMON, TEXT_COMMON, {
					"MaxVisibleGraphemes","AutoLocalize","LocalizationTable",
				})),
				TextButton = dedup(merge(COMMON, TEXT_COMMON, {
					"AutoButtonColor","Modal","Style","ContentImageLeft","ContentImageRight",
				})),
				...
				ImageButton = merge(COMMON, IMAGE_COMMON, {
					"HoverImage","PressedImage","Style","AutoButtonColor","Modal",
				}),
			...
			local function emitRaw(s) codeLines[#codeLines+1] = s end
			local function emitBlank() codeLines[#codeLines+1] = "" end
			...
			local function shouldSkip(propName, val)
				...
				if tv ~= td then return false end
				if tv == "Vector2" then return val.X == def.X and val.Y == def.Y end
				if tv == "UDim2" then
					return val.X.Scale == def.X.Scale and val.X.Offset == def.X.Offset
						and val.Y.Scale == def.Y.Scale and val.Y.Offset == def.Y.Offset
				end
				if tv == "UDim" then return val.Scale == def.Scale and val.Offset == def.Offset end
				if tv == "Color3" then
			...
			if #extractedScripts > 0 then
				...
				for i, sd in ipairs(extractedScripts) do
					emitRaw(("-- [%d] %s  (%s)  method: %s  enabled: %s"):format(
						i, sd.name, sd.className, sd.method, tostring(sd.enabled)))
			...
			for _, l in ipairs(codeLines) do
				if l:find("%.%a+ = ") then totalProps += 1 end
			end
			...
			if getgenv().DoNotif then
				getgenv().DoNotif(
					("✓ Deep GUI — %d elem, %d prop, %d script"):format(
						flatCounter.n, totalProps, #extractedScripts), 5)
			end
		end
		})
	...
	Lib.Window = (function()
		...
		local createGui = function(self)
			local gui = create({
				...
				{2,"Frame",{Active=true,BackgroundColor3=Color3.new(50,50,50),BackgroundTransparency=0.5,BorderSizePixel=0,Name="Main",Parent={1},Position=UDim2.new(0.40000000596046,0,0.40000000596046,0),Size=UDim2.new(0,300,0,300),}},
	...
	Lib.ContextMenu = (function()
		...
		local function createGui(self)
			local contextGui = create({
				...
				{2,"Frame",{Active=true,BackgroundColor3=Color3.new(50,50,50),BorderColor3=Color3.new(0.14117647707462,0.14117647707462,0.14117647707462),Name="Main",Parent={1},Position=UDim2.new(0.5,-100,0.5,-150),Size=UDim2.new(0,200,0,100),}},
				{3,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2},}},
				{4,"Frame",{BackgroundColor3=Color3.new(50,50,50),Name="Container",Parent={2},Position=UDim2.new(0,1,0,1),Size=UDim2.new(1,-2,1,-2),}},
...
BytecodeReader.__index = function(self, k)
  if k == "Position" then return rawget(self, "_pos") - 1 end
...
BytecodeReader.__newindex = function(self, k, v)
  if k == "Position" then
    ...
    if p < 1 or p > rawget(self, "Length") + 1 then
      error(("BytecodeReader: Position %d out of range [0,%d]"):format(v, rawget(self, "Length")), 2)
    end
    rawset(self, "_pos", p)
  else
    rawset(self, k, v)
...
local function _br_guard(self, n)
  if self._pos + n - 1 > self.Length then
    error(("BytecodeReader: EndOfStream — need %d byte(s) at offset %d (stream length %d)"):format(
      n, self._pos - 1, self.Length), 3)
  end
end
function BytecodeReader:ReadBoolean()
  _br_guard(self, 1)
  local b = string.byte(self.Stream, self._pos); self._pos = self._pos + 1
  if b == 0 then return false elseif b == 1 then return true
  else error(("BytecodeReader:ReadBoolean — non-boolean value: 0x%02X"):format(b), 2) end
end
function BytecodeReader:ReadByte()
  _br_guard(self, 1)
  local b = string.byte(self.Stream, self._pos); self._pos = self._pos + 1; return b
end
function BytecodeReader:ReadBytes(count)
  _br_guard(self, count)
  local t = {}
  for i = 1, count do t[i] = string.byte(self.Stream, self._pos); self._pos = self._pos + 1 end
  return t
end
function BytecodeReader:ReadInt16()
  _br_guard(self, 2)
  local a, b = string.byte(self.Stream, self._pos, self._pos + 1); self._pos = self._pos + 2
  local v = bor(a, bshl(b, 8)); if v >= 0x8000 then v = v - 0x10000 end; return v
end
...
function BytecodeReader:ReadUInt32()
  _br_guard(self, 4)
  local a, b, c, d = string.byte(self.Stream, self._pos, self._pos + 3); self._pos = self._pos + 4
  return bor(a, bor(bshl(b, 8), bor(bshl(c, 16), bshl(d, 24))))
end
function BytecodeReader:ReadInt32()
  local v = self:ReadUInt32(); if v >= 0x80000000 then v = v - 0x100000000 end; return v
end
function BytecodeReader:ReadUInt32Compressed()
  ...
  repeat
    _br_guard(self, 1)
    local b = string.byte(self.Stream, self._pos); self._pos = self._pos + 1
    result = bor(result, bshl(band(b, 0x7F), shift)); shift = shift + 7
    if band(b, 0x80) == 0 then break end
  until shift >= 35
  return result
end
function BytecodeReader:ReadInt32Compressed()
  local v = self:ReadUInt32Compressed(); if v >= 0x80000000 then v = v - 0x100000000 end; return v
end
function BytecodeReader:ReadSingle()
  ...
  if exp == 0 then return sign * (frac / 0x800000) * 2^-126
  elseif exp == 255 then return frac == 0 and (sign * math.huge) or (0/0)
  else return sign * (1 + frac / 0x800000) * 2^(exp - 127) end
end
function BytecodeReader:ReadDouble()
  _br_guard(self, 8)
  local bytes = { string.byte(self.Stream, self._pos, self._pos + 7) }; self._pos = self._pos + 8
  ...
  if exp == 0 then return sign * (frac / 2^52) * 2^-1022
  elseif exp == 0x7FF then return frac == 0 and sign * math.huge or (0/0)
  else return sign * (1 + frac / 2^52) * 2^(exp - 1023) end
end
...
function BytecodeReader:ReadASCII(length)
  ...
  local s = string.sub(self.Stream, self._pos, self._pos + length - 1); self._pos = self._pos + length; return s
end
function BytecodeReader:Peek()
  if self._pos > self.Length then return -1 end
...
function BytecodeReader:len() return self.Length end
function BytecodeReader:nextByte() return self:ReadByte() end
function BytecodeReader:nextSignedByte() local b = self:ReadByte(); return b >= 128 and b - 256 or b end
function BytecodeReader:nextBytes(n) return self:ReadBytes(n) end
function BytecodeReader:nextChar() return string.char(self:ReadByte()) end
function BytecodeReader:nextUInt32() return self:ReadUInt32() end
function BytecodeReader:nextInt32() return self:ReadInt32() end
function BytecodeReader:nextFloat() return self:ReadSingle() end
function BytecodeReader:nextDouble() return self:ReadDouble() end
function BytecodeReader:nextVarInt() return self:ReadUInt32Compressed() end
...
local pprint = (function()
  ...
  local _ppdepth = 1
  pprint.defaults = {
    depth_limit = 8, show_nil = true, show_boolean = true, show_number = true,
    show_string = true, show_table = true, show_function = true,
    show_thread = false, show_userdata = true,
    show_metatable = false, show_all = false, use_tostring = false,
    filter_function = nil, object_cache = 'local',
    indent_size = 2, level_width = 120, wrap_string = true, wrap_array = false,
    string_is_utf8 = false, sort_keys = true,
  }
  local TYPES = {['nil']=1,['boolean']=2,['number']=3,['string']=4,['table']=5,['function']=6,['thread']=7,['userdata']=8}
  local ESCAPE_MAP = {['\a']='\\a',['\b']='\\b',['\f']='\\f',['\n']='\\n',['\r']='\\r',['\t']='\\t',['\v']='\\v',['\\']='\\\\'}
  local function tokenize_string(s)
    local t = {}
    for i = 1, #s do
      local c = s:sub(i,i); local b = c:byte(); local e = ESCAPE_MAP[c]
      if (b >= 0x20 and b < 0x80) or e then t[i] = {char=e or c, len=#(e or c)}
      else t[i] = {char=string.format('\\x%02x',b), len=4} end
      if c == '"' then t.has_double_quote = true elseif c == "'" then t.has_single_quote = true end
    end
    return t
  end
  local tokenize_utf8_string = tokenize_string
  local function is_plain_key(key) return type(key)=='string' and key:match('^[%a_][%a%d_]*$') end
  local CACHE_TYPES = {['table']=true,['function']=true,['thread']=true,['userdata']=true}
  local function cache_apperance(obj, cache, option)
    if not cache.visited_tables then cache.visited_tables = setmetatable({},{__mode='k'}) end
    local t = type(obj)
    if (not TYPES[t] and not option.show_table) or (TYPES[t] and not option['show_'..t]) then return end
    if CACHE_TYPES[t] or TYPES[t] == nil then
      if not cache[t] then cache[t] = setmetatable({},{__mode='k'}); cache[t]._cnt = 0 end
      if not cache[t][obj] then cache[t]._cnt = cache[t]._cnt+1; cache[t][obj] = cache[t]._cnt end
    end
    if t == 'table' or TYPES[t] == nil then
      if cache.visited_tables[obj] == false then return
      elseif cache.visited_tables[obj] == nil then cache.visited_tables[obj] = 1
      else cache.visited_tables[obj] = cache.visited_tables[obj]+1; return end
      for k,v in pairs(obj) do cache_apperance(k,cache,option); cache_apperance(v,cache,option) end
      local mt = getmetatable(obj)
      if mt and option.show_metatable then cache_apperance(mt,cache,option) end
    end
  end
  local function str_natural_cmp(lhs,rhs)
    while #lhs>0 and #rhs>0 do
      local lmid,lend = lhs:find('%d+'); local rmid,rend = rhs:find('%d+')
      if not (lmid and rmid) then return lhs<rhs end
      local lsub,rsub = lhs:sub(1,lmid-1),rhs:sub(1,rmid-1)
      if lsub~=rsub then return lsub<rsub end
      local lnum,rnum = tonumber(lhs:sub(lmid,lend)),tonumber(rhs:sub(rmid,rend))
      if lnum~=rnum then return lnum<rnum end
      lhs=lhs:sub(lend+1); rhs=rhs:sub(rend+1)
    end
    return lhs<rhs
  end
  local function cmp(lhs,rhs)
    local tl,tr = type(lhs),type(rhs)
    if tl=='number' and tr=='number' then return lhs<rhs end
    if tl=='string' and tr=='string' then return str_natural_cmp(lhs,rhs) end
    if tl==tr then return str_natural_cmp(tostring(lhs),tostring(rhs)) end
  ...
  local function make_option(option)
    ...
    for k,v in pairs(pprint.defaults) do
      if option[k]==nil then option[k]=v end
      if option.show_all then
        for t,_ in pairs(TYPES) do option['show_'..t]=true end
        option.show_metatable=true
      end
    end
    return option
  end
  function pprint.setup(option) pprint.defaults = make_option(option) end
  function pprint.pformat(obj, option, printer)
    option = make_option(option)
    local buf = {}
    local function default_printer(s) table.insert(buf,s) end
    printer = printer or default_printer
    local cache
    if option.object_cache == 'global' then
      pprint._cache = pprint._cache or {}; cache = pprint._cache
    ...
    local function _p(s, no_inc)
      if not no_inc then printer(last); last=s; status.len=status.len+#s
      else last=last..s; status.len=status.len+#s end
    end
    local function _n()
      if last:match('^%s*$') then return false end
      printer(last..'\n'); last=string.rep(' ',status.len); status.len=0; return true
    end
    local function _indent(n)
      status.len = status.len + n
      if status.len < 0 then status.len = 0 end
    end
    local formatter = {}
    local function nop_formatter(_) return '' end
    local function tostring_formatter(v) return tostring(v) end
    ...
    local function make_fixed_formatter(t, use_cache)
      if use_cache then
        ...
        return function(v) return string.format('<%s>',t) end
      end
    end
    local function format(v) return formatter[type(v)](v) end
    local function string_formatter(s, force_long_quote)
      ...
      local string_len = 0
      for _,tok in ipairs(tokens) do string_len = string_len + tok.len end
      local long_quote_dashes = 0
      local function compute_long_quote_dashes()
        local pat = '%]' .. string.rep('=?', long_quote_dashes) .. '%]'
        while s:find(pat) do long_quote_dashes = long_quote_dashes + 1; pat = '%]' .. string.rep('=', long_quote_dashes) .. '%]' end
      end
      local quote_len = 2
      if force_long_quote then compute_long_quote_dashes(); quote_len = 2 + long_quote_dashes end
      if quote_len + string_len + status.len > option.level_width then
        ...
        if option.wrap_string and string_len + quote_len > option.level_width then
          if not force_long_quote then compute_long_quote_dashes(); quote_len = 2 + long_quote_dashes end
          ...
          for _,token in ipairs(tokens) do
            if line_len + token.len + status.len > option.level_width then
              _n(); _p(line, true); line_len = token.len; line = token.char
            else line_len = line_len + token.len; line = line .. token.char end
          end
          return line .. ']' .. dashes .. ']'
        end
      end
      if tokens.has_double_quote and tokens.has_single_quote and not force_long_quote then
        for i,token in ipairs(tokens) do if token.char=='"' then tokens[i].char='\\"' end end
      end
      local flat = {}; for _,token in ipairs(tokens) do table.insert(flat, token.char) end
      local concat = table.concat(flat)
      if force_long_quote then
        local dashes = string.rep('=', long_quote_dashes)
        return '[' .. dashes .. '[' .. concat .. ']' .. dashes .. ']'
      elseif tokens.has_single_quote then return '"' .. concat .. '"'
      else return "'" .. concat .. "'" end
    end
    local function table_formatter(t)
      if option.use_tostring then
        local mt = getmetatable(t)
        if mt and mt.__tostring then return string_formatter(tostring(t), true) end
      end
      ...
      if limit and _ppdepth > limit then
        if print_header_ix then return string.format('[[%s %d]]...', ttype, print_header_ix) end
        return string_formatter(tostring(t), true)
      end
      local tlen = #t
      local wrapped = false
      _p('{'); _indent(option.indent_size); _p(string.rep(' ', option.indent_size - 1))
      if print_header_ix then _p(string.format('--[[%s %d]] ', ttype, print_header_ix)) end
      for ix = 1, tlen do
        local v = t[ix]
        if formatter[type(v)] ~= nop_formatter and not (option.filter_function and option.filter_function(v,ix,t)) then
          if option.wrap_array then wrapped = _n() end
          _ppdepth = _ppdepth + 1; _p(format(v)..', '); _ppdepth = _ppdepth - 1
        end
      end
      local function is_hash_key(k)
        if type(k) ~= 'number' then return true end
        local numkey = math.floor(tonumber(k))
        if numkey ~= k or numkey > tlen or numkey <= 0 then return true end
      end
      local function print_kv(k,v,t)
        if formatter[type(v)] == nop_formatter or formatter[type(k)] == nop_formatter then return end
        if option.filter_function and option.filter_function(v,k,t) then return end
        wrapped = _n()
        if is_plain_key(k) then _p(k, true)
        else
          _p('[')
          local fk = format(k)
          if fk:match('%[%[') then _p(' '..fk..' ', true) else _p(fk, true) end
          _p(']')
        end
        _p(' = ', true); _ppdepth = _ppdepth + 1; _p(format(v), true); _ppdepth = _ppdepth - 1; _p(',', true)
      end
      if option.sort_keys then
        local keys = {}
        for k,_ in pairs(t) do if is_hash_key(k) then table.insert(keys, k) end end
        table.sort(keys, cmp)
        for _,k in ipairs(keys) do print_kv(k, t[k], t) end
      else
        for k,v in pairs(t) do if is_hash_key(k) then print_kv(k,v,t) end end
      end
      if option.show_metatable then
        local mt = getmetatable(t)
        if mt then print_kv('__metatable', mt, t) end
      end
      _indent(-option.indent_size)
      last = last:gsub('^ +$', '')
      last = last:gsub(',%s*$', ' ')
      if wrapped then _n() end
      _p('}')
      return ''
    end
    ...
    if option.object_cache then cache_apperance(obj, cache, option) end
    _p(format(obj)); printer(last)
    if option.object_cache == 'global' then pprint._cache = cache end
    return table.concat(buf)
  end
  function pprint.pprint(...)
    local args = {...}; local len = select('#', ...)
    for ix = 1, len do
      pprint.pformat(args[ix], nil, function(s) io.write(s) end)
      io.write('\n')
    end
  end
  ...
  local function tryCollapse(i)
    ...
    for j = i + 1, limit do
      ...
      elseif scanLine:match("^%s*$") then
        ...
        if scanLine:match("^%s*" .. ep .. "%s*=") then return false end
        local count = 0
        for _ in scanLine:gmatch(ep) do count = count + 1 end
        if count == 1 then
  ...
  local function tryFoldField(i)
    ...
    for j = i + 1, limit do
      ...
      elseif scanLine:match("^%s*$") then
        ...
        if scanLine:match("^%s*" .. epSrc .. "%s*=") then return false end
        if scanLine:match("^%s*" .. epReg .. "%s*=") then return false end
        local count = 0
        for _ in scanLine:gmatch(epReg) do count = count + 1 end
...
local function xorDecode(data, key)
  ...
  local klen = #key
  local kbytes = {}
  for i = 1, klen do kbytes[i] = string.byte(key, i) end
  local out = {}
  for i = 1, #data do
...
M.DEFAULT_OPTIONS = DEFAULT_OPTIONS
	...
	ScriptViewer.ViewScript = function(scr)
		...
		local function ZukPostProcess(src)
			...
			for iter = 1, 10 do
				...
				for i = 1, #lines - 1 do
					...
					if reg then
						...
						local j = i + 1
						while j <= #lines and (not lines[j] or lines[j]:match("^%s*$")) do j = j + 1 end
						if j <= #lines then
							local nline = lines[j]
							local lreg, field = nline:match("^(%s*)"..reg:gsub("%-","%-"):gsub("%(","%%("):gsub("%)","%%)").."([^=]+)=([^=].*)$")
			...
			for iter = 1, 6 do
				...
				for i = 1, #lines - 1 do
					...
					if not indent then continue end
					-- only inline simple literals / identifiers / field-chains (no function calls)
					local isSafe = not rhs:match("%(") and not rhs:match("%)")
						and not rhs:match("^local ")
					if not isSafe then continue end
					-- find next non-blank line
					local j = i + 1
					while j <= #lines and lines[j] and lines[j]:match("^%s*$") do j = j + 1 end
					if j > #lines or not lines[j] then continue end
					local nline = lines[j]
					-- count uses of reg in nline
					local count = 0
					for _ in nline:gmatch(reg) do count = count + 1 end
			...
			for i, line in ipairs(lines) do
				...
				if inDead then
					...
					if stripped:match("^end") or stripped:match("^else") 
					or stripped:match("^elseif") or stripped:match("^until") then
						inDead = false
					else
		...
		if okBC and bytecode and bytecode ~= "" then
			...
			if autoDecode then
				-- isValidLBC check: first byte should be LBC version 3-6
		...
		if ZukDecompile and okBC and bytecode and bytecode ~= "" then
			...
			local isValidResult = okD and type(result) == "string"
				and result ~= "" and not result:find("^%-%- DECOMPILER TIMEOUT")
...
--zukv2 core settings
			...
			env.decompile = function(scriptObj)
				...
				if okBC and bytecode and bytecode ~= "" then
					...
					if not isRawLBC and env.ZukAutoDecode then
						...
						if decoded and decoded ~= bytecode then bytecode = decoded end
					end
					local opts = {
						DecompilerMode        = "disasm",
						DecompilerTimeout     = 20,
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="Ugh",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=1,}},
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="A Better Dex.",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			{7,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="StatusText",Parent={3},Position=UDim2.new(0,20,0,110),Size=UDim2.new(0,180,0,25),Text="Grabbing balls.",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Improved",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			{20,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="AppName",Parent={18},Position=UDim2.new(0,2,0,38),Size=UDim2.new(1,-4,1,-40),Text="Explorer",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0,TextTruncate=1,TextWrapped=true,TextYAlignment=0,}},
