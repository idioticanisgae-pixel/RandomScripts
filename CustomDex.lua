		...
		local function main()
			...
			-- ── ZukPostProcess ─────────────────────────────────────────────────
				ZukPostProcess = function(src)
					...
					-- shared helpers
					local function nextNB(i)
						local j = i + 1
						while j <= #lines and (lines[j] == nil or lines[j]:match("^%s*$")) do j = j + 1 end
						return j
					end
					local function esc(s) return (s:gsub("([%(%)%.%%%+%-%*%?%[%^%$])", "%%%1")) end

					-- ── Pass 0: strip proto metadata noise ───────────────────────────
					local protoNoise = {
						"^%s*%-%- proto pool id:", "^%s*%-%- num upvalues:",
						"^%s*%-%- num inner protos:", "^%s*%-%- size instructions:",
						"^%s*%-%- size constants:", "^%s*%-%- lineinfo gap:",
						"^%s*%-%- max stack size:", "^%s*%-%- is typed:",
						"^%s*%-%- DECOMPILER REMARK:", "^%s*%-%- :[%a_][%w_]*%s*$",
					}
					for i, line in ipairs(lines) do
						if line then
							for _, pat in ipairs(protoNoise) do
								if line:match(pat) then lines[i] = nil; break end
							end
						end
					end

					-- ── Pass 1: vararg relay  `... = EXPR` + use of `(...)` ──────────
					for _ = 1, 12 do
						local changed = false
						for i = 1, #lines do
							local line = lines[i]
							if not line then continue end
							local ind, varExpr = line:match("^(%s*)%.%.%. = (.+)$")
							if not ind then continue end
							local j = nextNB(i)
							if j > #lines or not lines[j] then continue end
							local nline = lines[j]
							if nline:match("%(%.%.%.)") then
								lines[j] = nline:gsub("%(%.%.%.%)", "(" .. varExpr .. ")", 1)
								lines[i] = nil; changed = true
							elseif nline:match("^%s*return%s+[%w_%.]+,%s*%.%.%.%s*$") and varExpr:match(":") then
								local ind2 = nline:match("^(%s*)")
								lines[j] = ind2 .. "return " .. varExpr
								lines[i] = nil; changed = true
							end
						end
					...
					-- ── Pass 2: `vN = FUNC` + `DEST = vN(ARGS)` ─────────────────────
					for _ = 1, 12 do
						local changed = false
						for i = 1, #lines do
							local line = lines[i]
							if not line then continue end
							local ind, reg, funcExpr = line:match("^(%s*)(v%d+) = ([%a_][%w_%.%:]*)$")
							if not ind or funcExpr:match("^v%d") then continue end
							local j = nextNB(i)
							if j > #lines or not lines[j] then continue end
							local nline = lines[j]
							local ind2, lhs, args = nline:match("^(%s*)([%w_%.]+) = " .. esc(reg) .. "%((.*)%)$")
							if ind2 then
								lines[j] = ind2 .. lhs .. " = " .. funcExpr .. "(" .. args .. ")"
								lines[i] = nil; changed = true
							end
						end
						if not changed then break end
					...
					-- ── Pass 3: field-chain  `vN = vM` + `vN = vN.X` ────────────────
					for _ = 1, 12 do
						local changed = false
						for i = 1, #lines do
							local line = lines[i]
							if not line then continue end
							local ind, lhs, rhs = line:match("^(%s*)(v%d+) = (v%d+)$")
							if not ind then continue end
							local j = nextNB(i)
							if j > #lines or not lines[j] then continue end
							local nline = lines[j]
							local ind2, lhs2, tail = nline:match("^(%s*)(v%d+) = " .. esc(lhs) .. "([%.%:][%a_][%w_%.:%(%)'\"%s,]*)$")
							if ind2 and lhs2 == lhs then
								lines[j] = ind2 .. lhs .. " = " .. rhs .. tail
								lines[i] = nil; changed = true
							end
						end
						if not changed then break end
					...
					-- ── Pass 4: inline single-use simple regs ────────────────────────
					for _ = 1, 16 do
						local changed = false
						for i = 1, #lines do
							local line = lines[i]
							if not line then continue end
							local ind, reg, rhs = line:match("^(%s*)(v%d+) = (.+)$")
							if not ind then continue end
							if rhs:match("^{") or rhs:match("^local ") then continue end
							local j = nextNB(i)
							if j > #lines or not lines[j] then continue end
							local nline = lines[j]
							local count = 0
							for _ in nline:gmatch("%f[%w_]" .. esc(reg) .. "%f[^%w_]") do count = count + 1 end
							local isAssigned    = nline:match("^%s*" .. esc(reg) .. "%s*=") ~= nil
							local isFieldTarget = nline:match("^%s*" .. esc(reg) .. "%.[%a_][%w_]*%s*=") ~= nil
							if count == 1 and not isAssigned and not isFieldTarget then
								local wrap = rhs
								if rhs:match("%s+[%+%-%*/%%]%s+") then wrap = "(" .. rhs .. ")" end
								lines[j] = nline:gsub("%f[%w_]" .. esc(reg) .. "%f[^%w_]", wrap, 1)
								lines[i] = nil; changed = true
							end
						end
						if not changed then break end
					...
					-- ── Pass 5: `return vN, ...` cleanup ─────────────────────────────
					for i, line in ipairs(lines) do
						if not line then continue end
						local ind2, r = line:match("^(%s*)return%s+([%w_%.]+),%s*%.%.%.%s*$")
						if not ind2 then continue end
						for k = i - 1, math.max(1, i - 5), -1 do
							if lines[k] then
								local ce = lines[k]:match("^%s*%.%.%. = (" .. esc(r) .. "%b())$")
								if ce then
									lines[i] = ind2 .. "return " .. ce
									lines[k] = nil
								end
								break
							end
					...
					-- ── Pass 6: `false +1` → `true` ──────────────────────────────────
					for i, line in ipairs(lines) do
						if line then lines[i] = line:gsub("false %+1", "true") end
					end

					-- ── Pass 7: strip dead code after return ──────────────────────────
					local inDead = false
					for i, line in ipairs(lines) do
						if not line then continue end
						local s = line:match("^%s*(.-)%s*$")
						if inDead then
							if s:match("^end") or s:match("^else") or s:match("^elseif") or s:match("^until") then
								inDead = false
							elseif s ~= "" and not s:match("^%-%-") then
								lines[i] = nil
							end
						end
						if not inDead and s:match("^return[%s%(]") then inDead = true end
					end

					-- ── Pass 8: strip `local vN = nil` before for ────────────────────
					for i = 1, #lines - 1 do
						if not lines[i] then continue end
						if lines[i]:match("^%s*local%s+v%d+%s*=%s*nil%s*$") then
							local j = nextNB(i)
							if lines[j] and lines[j]:match("^%s*for%s+") then lines[i] = nil end
						end
					end

					-- ── Pass 9: upvalue name resolution ──────────────────────────────
					-- Build a name map from every pattern we can observe:
					--   upv_N used as :GetModule("Name")  → upv_Name
					--   upv_N used as .someField          → upv_someField
					--   upv_N = game:GetService("Svc")    → upv_svc
					--   upv_N = players.LocalPlayer       → upv_localPlayer
					local upvMap = {}
					for _, line in ipairs(lines) do
						if not line then continue end
						-- :GetModule("Name")
						local uv, mod = line:match("[^%w_](upv_%d+)[^%w_].*:GetModule%([\"']([%w_]+)[\"']%)")
						if not uv then uv, mod = line:match("^%s*(upv_%d+)%s*=.*:GetModule%([\"']([%w_]+)[\"']%)") end
						if uv and mod and not upvMap[uv] then upvMap[uv] = mod:sub(1,1):lower()..mod:sub(2) end
						-- game:GetService("Name")
						local uv2, svc = line:match("^%s*(upv_%d+)%s*=.*GetService%([\"']([%w_]+)[\"']%)")
						if uv2 and not upvMap[uv2] then upvMap[uv2] = svc:sub(1,1):lower()..svc:sub(2) end
						-- upv_N = something.fieldName
						local uv3, fld = line:match("^%s*(upv_%d+)%s*=[^%.]*%.([%a_][%w_]*)%s*$")
						if uv3 and not upvMap[uv3] then upvMap[uv3] = fld end
						-- upv_N = require(...)  → upv_module (generic)
						local uv4 = line:match("^%s*(upv_%d+)%s*= require%b()")
						if uv4 and not upvMap[uv4] then upvMap[uv4] = "module" end
					end
					if next(upvMap) then
						for i, line in ipairs(lines) do
							if not line then continue end
							for uv, name in pairs(upvMap) do
								lines[i] = lines[i]:gsub(esc(uv), "upv_"..name)
							end
						end
					end

					-- ── Pass 10: param `self` inference ──────────────────────────────
					-- If a function's first param (p1/p2/etc.) is ONLY ever used as
					-- `pN.field` or `pN:method(` and never reassigned, it's `self`.
					-- Strategy: scan each function body for pN usage patterns.
					do
						...
						for i, line in ipairs(lines) do
							if not line then continue end
							local s = line:match("^%s*(.-)%s*$")
							-- local function Foo(pN)  or  function(pN)
							local p = s:match("^local%s+function%s+[%w_]+%(([%w_,% ]-)%)")
								or s:match("^function%s+[%w_%.]+%(([%w_,% ]-)%)")
								or s:match("^local%s+function%s+[%w_]+%s*%(([%w_,% ]-)%)")
							if p then
								-- first param
								local firstParam = p:match("^%s*([%w_]+)")
								if firstParam and firstParam:match("^p%d+$") then
									table.insert(funcRanges, {startLine=i, param=firstParam, endLine=nil})
								end
							end
							if s:match("^end%s*$") or s == "end" then
								if #funcRanges > 0 and funcRanges[#funcRanges].endLine == nil then
									funcRanges[#funcRanges].endLine = i
								end
							end
						end
						-- for each function range, check if param is self-like
						for _, fr in ipairs(funcRanges) do
							if not fr.endLine then continue end
							local p = fr.param
							local ep = esc(p)
							local isSelf = true
							local used = false
							for i = fr.startLine + 1, fr.endLine - 1 do
								local line = lines[i]
								if not line then continue end
								-- any use of pN that is NOT pN.X or pN:X → not self
								-- strip field-access and method-call patterns then check if pN remains
								local stripped = line
									:gsub("%f[%w_]" .. ep .. "%f[^%w_][%.%:][%a_][%w_]*", "")
								if stripped:match("%f[%w_]" .. ep .. "%f[^%w_]") then
									-- pN appears in a non-OOP context
									isSelf = false; break
								else
									-- check it was actually used in OOP context
									if line:match("%f[%w_]" .. ep .. "%f[^%w_][%.%:]") then
										used = true
									end
								end
							end
							if isSelf and used then
								-- rename pN → self in this range
								for i = fr.startLine, fr.endLine do
									if lines[i] then
										lines[i] = lines[i]:gsub("%f[%w_]" .. ep .. "%f[^%w_]", "self")
									end
								end
							end
						end
					...
					-- ── Pass 11: vN register rename heuristics (per-function scope) ──
				do
					local kw11 = {["and"]=1,["break"]=1,["do"]=1,["else"]=1,["elseif"]=1,["end"]=1,["false"]=1,["for"]=1,["function"]=1,["if"]=1,["in"]=1,["local"]=1,["nil"]=1,["not"]=1,["or"]=1,["repeat"]=1,["return"]=1,["then"]=1,["true"]=1,["until"]=1,["while"]=1,["self"]=1}
					local function deriveName11(rhs)
						local field = rhs:match("^[%w_%.]+%.([%a_][%w_]*)$")
						if field and not field:match("^v%d") then return field:sub(1,1):lower()..field:sub(2) end
						local svc = rhs:match("GetService%([\"']([%w_]+)[\"']%)")
						if svc then return svc:sub(1,1):lower()..svc:sub(2) end
						return nil
					end
					local ranges11 = {{startLine=1, endLine=#lines}}
					for i, line in ipairs(lines) do
						if not line then continue end
						local s = line:match("^%s*(.-)%s*$")
						local sc = s:gsub('"%.-"', ""):gsub("'.-'", "")
						if sc:match("^local%s+function%s+[%w_]") or sc:match("^function%s+[%w_]") or sc:match("%f[%w_]function%f[^%w_]%s*%(") then
							ranges11[#ranges11 + 1] = {startLine=i, endLine=nil}
						end
						if s == "end" or s:match("^end[%s,%)%;%]]") then
							for ri = #ranges11, 2, -1 do
								if not ranges11[ri].endLine then ranges11[ri].endLine = i; break end
							end
						end
					end
					ranges11[1].endLine = #lines
					for _, rng in ipairs(ranges11) do
						local sl, el = rng.startLine, rng.endLine or #lines
						local aC, aR, aL = {}, {}, {}
						for i = sl, el do
							local line = lines[i]
							if not line then continue end
							local reg, rhs = line:match("^%s*(v%d+) = (.+)$")
							if reg then
								aC[reg] = (aC[reg] or 0) + 1
								if aC[reg] == 1 then aR[reg]=rhs; aL[reg]=i else aR[reg]=nil end
							end
						end
						local usedN, renames = {}, {}
						for reg, rhs in pairs(aR) do
							if rhs then
								local name = deriveName11(rhs)
								if name and not usedN[name] and not kw11[name] then usedN[name]=true; renames[reg]=name end
							end
						end
						for reg, name in pairs(renames) do
							local ep = esc(reg)
							for i = sl, el do
								local line = lines[i]
								if line then
									if i == aL[reg] then
										lines[i] = line:gsub("^(%s*)" .. ep .. "(%s*=)", "%1local " .. name .. "%2", 1)
									else
										lines[i] = line:gsub("%f[%w_]" .. ep .. "%f[^%w_]", name)
									end
								end
							end
						end
					...
					-- ── Pass 12: `tbl.Key = vN` where vN is a named local function ────
					-- zukv2 emits:  local function Generate() ... end
					--               v1.Generate = v2   ← v2 should be Generate
					-- If the field name matches a local function name we've seen, fix it.
					do
						local funcNames = {}
						for _, line in ipairs(lines) do
							if not line then continue end
							local fname = line:match("^%s*local%s+function%s+([%a_][%w_]*)%s*%(")
							if fname then funcNames[fname] = true end
						end
						for i, line in ipairs(lines) do
							if not line then continue end
							-- tbl.Key = vN  where Key is a known function name
							local tbl, key, reg = line:match("^(%s*[%w_%.]+)%.([%a_][%w_]*)%s*=%s*(v%d+)%s*$")
							if tbl and key and reg and funcNames[key] then
								lines[i] = tbl .. "." .. key .. " = " .. key
							end
						end
					end

					-- ── Pass 13: clean up `local v0 = v0` self-assignments from scope reset ─
					for i, line in ipairs(lines) do
						if not line then continue end
						local reg = line:match("^%s*local%s+(v%d+)%s*=%s*(v%d+)%s*$")
						if reg then
							local lhs, rhs = line:match("^%s*local%s+(v%d+)%s*=%s*(v%d+)%s*$")
							if lhs == rhs then lines[i] = nil end
						end
					end

					-- ── Pass 14: reconstruct table literals from vN={} + vN.K=val ──
					do
						local compact = {}
						for _, l in ipairs(lines) do if l ~= nil then compact[#compact+1] = l end end
						local function isSimpleVal(s)
							if not s or s == "" then return false end
							-- reject bare register names like v0, v12 — these are unresolved refs
							if s:match("^v%d+$") then return false end
							if s:match('^".-"$') or s:match("^'.-'$") then return true end
							if s:match("^%-?%d+%.?%d*$") then return true end
							if s == "true" or s == "false" or s == "nil" then return true end
							-- function call: identifier.chain(args) - balanced parens
							if s:match("^[%a_][%w_%.]*%b()$") then return true end
							-- simple identifier chain with no parens (e.g. Enum.NormalId.Back)
							if s:match("^[%a_][%w_%.]+$") then return true end
							return false
						end
						local tblDecl, tblField, tblDead, tblComment = {}, {}, {}, {}
						for i, line in ipairs(compact) do
							local s = line:match("^%s*(.-)%s*$")
							local reg = s:match("^([%a_][%w_]*)%s*=%s*{}") or s:match("^local%s+([%a_][%w_]*)%s*=%s*{}")
							if reg then
								local comment = s:match("{}(.*)$") or ""
								tblDecl[reg] = i; tblField[reg] = {}; tblDead[reg] = false
								tblComment[reg] = comment:match("^%s*(.-)%s*$")
							end
							local tbl, key, val
							tbl, key, val = s:match("^([%a_][%w_]*)%.([%a_][%w_]*)%s*=%s*(.+)$")
							if not tbl then tbl, key, val = s:match('^([%a_][%w_]*)%["([^"]+)"%]%s*=%s*(.+)$') end
							if not tbl then
								local t2, k2, v2 = s:match("^([%a_][%w_]*)%[(%d+)%]%s*=%s*(.+)$")
								if t2 then tbl, key, val = t2, "["..k2.."]", v2 end
							end
							if tbl and tblField[tbl] and not tblDead[tbl] and isSimpleVal(val) then
								local f = tblField[tbl]; f[#f+1] = {key=key, val=val, lineIdx=i}
							end
							local ra = s:match("^([%a_][%w_]*)%s*=[^%.]")
							if ra and tblDecl[ra] and tblDecl[ra] ~= i then tblDead[ra] = true end
						end
						local lineReplace = {}
						for reg, fields in pairs(tblField) do
							if #fields < 2 then continue end
							local declIdx = tblDecl[reg]
							local declLine = compact[declIdx]
							local indent = declLine:match("^(%s*)") or ""
							local isLocal = declLine:match("^%s*local%s+") ~= nil
							local prefix = isLocal and ("local " .. reg) or reg
							local comment = tblComment[reg] or ""
							local block = {}
							block[#block+1] = indent .. prefix .. " = {" .. (comment ~= "" and ("  " .. comment) or "")
							for fi, f in ipairs(fields) do
								local comma = fi < #fields and "," or ""
								local keyStr = f.key:match("^%[") and (f.key.." = ") or (f.key.." = ")
								block[#block+1] = indent .. "    " .. keyStr .. f.val .. comma
							end
							block[#block+1] = indent .. "}"
							lineReplace[declIdx] = table.concat(block, "\n")
							for _, f in ipairs(fields) do lineReplace[f.lineIdx] = false end
						end
						if next(lineReplace) then
							local rebuilt = {}
							for i, line in ipairs(compact) do
								local rep = lineReplace[i]
								if rep == false then -- deleted
								elseif rep ~= nil then rebuilt[#rebuilt+1] = rep
								else rebuilt[#rebuilt+1] = line end
							end
							lines = rebuilt
						end
					end
				...
				if ZukDecompile and okBC and bytecode and bytecode ~= "" then
					...
					if isValidResult then
						...
						source = source .. "-- Decompiled in " .. tostring(math.floor((tick() - oldtick) * 100) / 100) .. "s\n\n"
