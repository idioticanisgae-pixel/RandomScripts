...
function RunCustomAnimation(Char)
			...
			if AnimationSpeedDampeningObject ~= nil then
				scale = 1
					+ (Humanoid.HipHeight - HumanoidHipHeight)
						* AnimationSpeedDampeningObject.Value
						/ HumanoidHipHeight
			end
			return scale
		end
	...
	local function updateVelocity(currentTime)
		...
		if locomotionMap == strafingLocomotionMap then
			...
			if
				...
				or math.abs(humanoidSpeed - cachedRunningSpeed) > 0.01
				or currentTime - lastBlendTime > 1
			then
	...
	local function updateVelocityBounds(velo)
		if velo then
			if velo.x > maxVeloX then
				maxVeloX = velo.x
			end
			if velo.y > maxVeloY then
				maxVeloY = velo.y
			end
			if velo.x < minVeloX then
				minVeloX = velo.x
			end
			if velo.y < minVeloY then
				minVeloY = velo.y
			end
		end
	end
